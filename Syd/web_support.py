...
# CONNECTION_CODE=your-initial-6-digit-code
...
async def send(request):
    ...
    item = {
        ...
        "kind": kind,
        "text": text,
        "timestamp": datetime.now(timezone(timedelta(hours=5, minutes=30))).strftime("%d %b %Y, %I:%M %p") + " IST"
    }
...
async def history(request):
    # Public history for the configured personal relay. The app can show
    # existing chat history before a device connects.
    if not sessions:
        return web.json_response({"history": []})
    requested = request.query.get("code", "").strip()
    if requested and requested in sessions:
        return web.json_response({"history": sessions[requested]["history"]})
    history_items = []
    for sess in sessions.values():
        history_items.extend(sess["history"])
    history_items.sort(key=lambda x: x.get("timestamp", ""))
    return web.json_response({"history": history_items[-200:]})
